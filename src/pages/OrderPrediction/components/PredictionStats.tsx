...
export default function PredictionStats({ modelStats }: PredictionStatsProps) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold mb-4">Performances du Modèle</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-medium">{modelStats.rmse?.toFixed(2) || 'N/A'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-600 dark:text-slate-400">MAE:</span>
              <span className="font-medium">{modelStats.mae?.toFixed(2) || 'N/A'}</span>
            </div>
          </div>
          <div className="space-y-2">
            <div className="flex justify-between">
              <span className="text-slate-600 dark:text-slate-400">Niveau de Confiance:</span>
              <span className="font-medium">{modelStats.confidence_level ? `${(modelStats.confidence_level * 100).toFixed(1)}%` : 'N/A'}</span>
            </div>
          </div>
        </div>
      </div>
